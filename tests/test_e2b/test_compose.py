"""Tests for E2B compose configuration conversion."""
...
from inspect_sandboxes.e2b._compose import (
    DEFAULT_CPU_COUNT,
    DEFAULT_MEMORY_MB,
...
@pytest.mark.parametrize(
    ...
    [
        ({}, None),
        ({"x-e2b": {}}, None),
        ({"x-e2b": {"timeout": 1800}}, 1800.0),
        # `timeout` must come through even when other x-e2b keys are present
        ({"x-e2b": {"timeout": 30, "template": "foo"}}, 30),
        # Numeric strings (what YAML produces from quoted values) must coerce.
        ({"x-e2b": {"timeout": "45"}}, 45.0),
        ({"x-e2b": {"timeout": "45.5"}}, 45.5),
        # int values are fine (YAML unquoted ints).
        ({"x-e2b": {"timeout": 0}}, 0.0),
    ],
)
def test_extract_e2b_timeout(
    extensions: dict[str, Any], expected: float | None
) -> None:
    """Test that x-e2b.timeout is extracted independently of the params dict."""
    result = extract_e2b_timeout(extensions)
...
def test_extract_e2b_timeout_rejects_non_numeric(bad_value: Any) -> None:
    """Values that can't coerce to float must raise ValueError with context."""
    with pytest.raises(ValueError, match="x-e2b.timeout must be a number"):
        extract_e2b_timeout({"x-e2b": {"timeout": bad_value}})
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # x-e2b overrides take precedence
        (
            {"image": "alpine"},
...
def test_convert_compose_missing_dockerfile() -> None:
    ...
    with patch(
        "inspect_sandboxes.e2b._compose.resolve_dockerfile_path"
    ) as mock_resolve:
