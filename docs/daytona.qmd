...
**DinD snapshot names** are deterministic, derived from aggregated per-service resources and the DinD image: `inspect-dind-<cpu>cpu-<mem>gb-<gpu>gpu-<image-hash>` (e.g. `inspect-dind-2cpu-4gb-0gpu-bb4ebfa28b94`). The trailing 12-char hex is a hash of `DIND_IMAGE`, so bumping the image invalidates the cache and a fresh snapshot is built. Samples with matching resource profiles and image reuse the same snapshot.
